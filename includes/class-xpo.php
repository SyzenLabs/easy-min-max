...
class Xpo {
	...
	public static function generate_utm_link( $params = array() ) {
		...
		$base_url      = $params['url'] ?? 'https://www.wpxpo.com/table-rate-shipping-for-woocommerce/';
		$utm_key       = $params['utmKey'] ?? null;
		$affiliate     = $params['affiliate'] ?? apply_filters( 'eamm_affiliate_id', '' );
		...
		if ( $hash ) {
			$final_url .= ( strpos( $hash, '#' ) === false ? '#' : '' ) . $hash;
		}
