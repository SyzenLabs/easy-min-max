...
class DurbinClient {
	...
	const PLUGIN_SLUG = 'easy-min-max';
	...
	public static function send( $action_type ) {
		...
		if ( self::DEACTIVATE_ACTION === $action_type ) {
			...
			if ( ! empty( $id ) ) {
				$data['feedback'] = array(
					'id'      => $id, 'details' => isset( $_POST['cause_details'] ) ? sanitize_text_field( wp_unslash( $_POST['cause_details'] ) ) : null, // phpcs:ignore
				);
			}
		}
